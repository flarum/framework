...
class User implements ExtenderInterface
{
    ...
    {
        $container->extend(
            'flarum.user.display_name.supported_drivers',
            fn ($existingDrivers) => array_merge($existingDrivers, $this->displayNameDrivers)
        );

        $container->extend(
            'flarum.user.group_processors',
            fn ($existingRelations) => array_merge($existingRelations, $this->groupProcessors)
        );
