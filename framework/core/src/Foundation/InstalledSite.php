...
class InstalledSite implements SiteInterface
{
    ...
    {
        $container->singleton('cache.store', fn ($container) => new CacheRepository($container->make('cache.filestore')));
        $container->alias('cache.store', Repository::class);

        $container->singleton('cache.filestore', fn () => new FileStore(new Filesystem, $this->paths->storage.'/cache'));
