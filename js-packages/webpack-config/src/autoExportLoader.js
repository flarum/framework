...
function addAutoExports(source, pathToModule, moduleName) {
  ...
  // In a normal case, we do one of two things:
    ...
    //              (checkout the `common/utils/string.ts` file for an example).
      ...
      // Find all `export x` in the file.
      const matches = [...source.matchAll(/export\s+?(?:\* as|function|const|abstract\s?|class)+?\s([A-Za-z_]*)/gm)];
      ...
      // 2.2. If there is no default export, then there are named exports,
      //      so we add the module to the registry with the map of named exports.
      ...
      //                and can be imported using `import { A, B } from 'flarum/../nanoid'`.
        ...
        const namedExportMatches = [...source.matchAll(/export\s+?{(.*)}\s+?from\s+?['"](.*)['"]/gm)];

        if (namedExportMatches.length) {
          source = source.replace(/export\s+?{(.*)}\s+?from\s+?['"](.*)['"]/gm, (match, names, path) => {
            return names
              .split(',')
          ...
          const map = namedExportMatches.reduce((map, match) => {
...
module.exports = function autoExportLoader(source) {
  ...
  // is updated.
  const composerJsonPath = path.resolve('../composer.json');
