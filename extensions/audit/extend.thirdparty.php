...
return [
    (new Conditional())
        ->whenExtensionEnabled('clarkwinkelmann-author-change', fn () => [
            (new Audit())
                ->group('clarkwinkelmann-author-change')
                ->listen(AuthorChangeEvent\DiscussionCreateDateChanged::class, 'discussion.create_date_changed', fn ($e) => [
                    'discussion_id' => $e->discussion->id,
                    'old_date' => $e->oldDate->toIso8601String(),
                    'new_date' => $e->discussion->created_at->toIso8601String(),
                ])
                ->listen(AuthorChangeEvent\DiscussionUserChanged::class, 'discussion.user_changed', fn ($e) => [
                    'discussion_id' => $e->discussion->id,
                    'old_user_id' => optional($e->oldUser)->id,
                    'new_user_id' => optional($e->discussion->user)->id,
                ])
                ->listen(AuthorChangeEvent\PostCreateDateChanged::class, 'post.create_date_changed', fn ($e) => [
                    'post_id' => $e->post->id,
                    'discussion_id' => $e->post->discussion->id,
                    'old_date' => $e->oldDate->toIso8601String(),
                    'new_date' => $e->post->created_at->toIso8601String(),
                ])
                ->listen(AuthorChangeEvent\PostEditDateChanged::class, 'post.edit_date_changed', fn ($e) => [
                    'post_id' => $e->post->id,
                    'discussion_id' => $e->post->discussion->id,
                    'old_date' => optional($e->oldDate)->toIso8601String(),
                    'new_date' => optional($e->post->edited_at)->toIso8601String(),
                ])
                ->listen(AuthorChangeEvent\PostUserChanged::class, 'post.user_changed', fn ($e) => [
                    'post_id' => $e->post->id,
                    'discussion_id' => $e->post->discussion->id,
                    'old_user_id' => optional($e->oldUser)->id,
        ...
        ->whenExtensionEnabled('fof-ban-ips', fn () => [
            (new Audit())
                ->group('fof-ban-ips')
                ->listen(BanIPsEvents\IPWasBanned::class, 'fof_ban_ips.banned', fn ($e) => array_filter([
                    'ip' => $e->bannedIP->address,
                    'reason' => $e->bannedIP->reason,
                    'user_id' => $e->bannedIP->user_id ?: null,
                ], fn ($v) => $v !== null))
                ->listen(BanIPsEvents\IPWasUnbanned::class, 'fof_ban_ips.unbanned', fn ($e) => array_filter([
                    'ip' => $e->unbannedIP->address,
                    'user_id' => $e->unbannedIP->user_id ?: null,
                ], fn ($v) => $v !== null)),
        ])
        ->whenExtensionEnabled('fof-impersonate', fn () => [
            (new Audit())
        ...
        ->whenExtensionEnabled('fof-username-request', fn () => [
            (new Audit())
