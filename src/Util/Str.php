 ...
 * Usage: $slug = Str::slug( 'Cómò ünç 中文百强网' , md5(uniqid(rand(), true)) );
 *
 * @author: Zeokat 
 *
...
class Str
{
     ...
     * @param string $fallback_slug Optional. A slug to use when empty slug is returned.
     * @return string The sanitized slug with unicode support.
     */
    public static function slug( $title, $fallback_slug = '' )
    {
        // Replace accents and create sanitized slug
        $slug = (new self)->sanitize_slug_string((new self)->convert_accents($title));

        // Use fallback_slug when the result of sanitization is an empty slug
        if ( '' === $slug || false === $slug ) {
            $slug = $fallback_slug;
        }
     ...
     * @return string The sanitized slug with unicode support.
     */
    function sanitize_slug_string ( $str ) {
        ...
        $str = str_replace( array( '&nbsp;', '&#160;', '%c2%a0', '&ndash;', '&#8211;', '%e2%80%93',
            '&mdash;', '&#8212;', '%e2%80%94' ), '-', $str );
     ...
     */
    function url_encode_unicode_chars( $utf8_string ) {
        $encoded_str = '';
        $values = array();
        $octets = 1;
